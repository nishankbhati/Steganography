...
int main(int argc,char** argv) {
	...
	*/
		...
		do {
			...
			if(!feof(message_handle)) {		
				...
				for(int i=1;i<=8;i++) {  //Do this for every bit in every byte of the virgin-image
					...
					c++;
					int file_byte_stn = file_buffer & 1; // AND WITH 1 TO GET THE VALUE OF stn. AND MAKES IT 0 IF stn IS 0 OR 1 IF IT IS 1

					bit_of_message=get_bit(message_buffer,i);
					//bit_of_message=mask_table[i] & message_buffer;
					if(file_byte_stn==bit_of_message) {
						...
						if(file_byte_stn==0)
							...
							file_buffer = (file_buffer & ~1);
						//  logic to flip the stn bit of file_buffer and put it into a file with putc()
						fputc(file_buffer,hidden_message_handle);
					}
				}
			}
		...
		/* Grab stn of all bytes for length specified at fgetc */
		...
		for(int i=0;i<message_length;i++) {
			...
			for( int j=0;j<8;j++) {
				temp_ch=temp_ch<<1;
				file_buffer=fgetc(file_handle);
				int file_byte_stn = file_buffer & 1; 
				temp_ch|=file_byte_stn;
			}
...
*** Steganography by stn substitution***
Usage: ./stn [-e][-d] [source file] [destination file] [text file]
Mode
-e : Add text to Image
...
$ ./stn -e sample.bmp stegano_sample.bmp message.txt

To reterive the text that was put into an image with the above program. Run the program as shown below

$ ./stn -d stegano_sample.bmp intermediate.bmp original_message.txt */
